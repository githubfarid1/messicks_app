...
class GrabUrlsVendor2Frame(ttk.Frame):
	def __init__(self, window) -> None:
		super().__init__(window)
		# configure
		self.grid(column=0, row=0, sticky=(N, E, W, S), columnspan=4)
		...
		# populate
		titleLabel = TitleLabel(self, text="Grab URLs By Vendor ver.2")
		closeButton = CloseButton(self)
	...
	def run_process(self, **kwargs):
			# messagebox.showwarning(title='Warning', message='')
			run_module(comlist=[PYLOC, "modules/urlsmessick_vendor_xls_v2.py"])

class MessickPdfDownload4Frame(ttk.Frame):
	def __init__(self, window) -> None:
		super().__init__(window)
		# configure
		self.grid(column=0, row=0, sticky=(N, E, W, S), columnspan=4)
		...
		# populate
		titleLabel = TitleLabel(self, text="Download PDF Diagram by File input ver.2")
		odsInputFile = FileChooserFrame(self, btype="file", label="Select XLS Input File:", filetypes=(("xls files", "*.xlsx"),("all files", "*.*")))
		closeButton = CloseButton(self)
		runButton = ttk.Button(self, text='Run Process', command = lambda:self.run_process(input=odsInputFile.filename))
	...
	def run_process(self, **kwargs):
			# messagebox.showwarning(title='Warning', message='')
			run_module(comlist=[PYLOC, "modules/pdfdownload_xls_booster_v2.py", "-i", kwargs['input']])
